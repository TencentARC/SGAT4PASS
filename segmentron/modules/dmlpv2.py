...
class DWConv2d(nn.Module):
    def __init__(self, in_chans, out_chans, kernel_size=3, padding=1, bias=False):
        ...
        self.pointwise = nn.Conv2d(
            in_chans, out_chans, kernel_size=1, bias=bias)
...
class SEMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None,
                 act_layer=nn.GELU, drop=0., linear=False, use_se=True):
        super().__init__()
        ...
        self.gamma = nn.Parameter(torch.ones(
            hidden_features), requires_grad=True)
        self.act = act_layer()
        ...
        self.se = SqueezeExcite(
            out_features, se_ratio=0.25) if use_se else nn.Identity()
    ...
    def forward(self, x, H, W):
        ...
        x = self.fc2(x)
        x = self.drop(x)
        x = self.se(x.permute(0, 2, 1).reshape(B, C, H, W)
...
class DeformableMLP(nn.Module):
    def __init__(
        ...
        self.weight = nn.Parameter(torch.empty(
            out_channels, in_channels // groups, 1, 1))  # kernel size == 1
...
class DeformableMLPBlock(nn.Module):
    def __init__(self, in_chans=512, emb_chans=64, drop_path=0.):
        ...
        self.pooling = nn.AvgPool2d((h, w), stride=1, padding=(
            h//2, w//2), count_include_pad=False)
