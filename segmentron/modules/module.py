...
class _ASPP(nn.Module):
    def __init__(self, in_channels=2048, out_channels=256):
        ...
        self.aspp1 = SeparableConv2d(
            in_channels, out_channels, dilation=dilations[0], relu_first=False)
        self.aspp2 = SeparableConv2d(
            in_channels, out_channels, dilation=dilations[1], relu_first=False)
        self.aspp3 = SeparableConv2d(
            in_channels, out_channels, dilation=dilations[2], relu_first=False)
    ...
    def forward(self, x):
        ...
        pool = F.interpolate(
            pool, size=x.shape[2:], mode='bilinear', align_corners=True)
...
class PyramidPooling(nn.Module):
    def __init__(self, in_channels, sizes=(1, 2, 3, 6), norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        for size in sizes:
            ...
            self.convs.append(_ConvBNReLU(
                in_channels, out_channels, 1, norm_layer=norm_layer, **kwargs))
